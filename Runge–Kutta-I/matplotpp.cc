...
dvec Axes::make_tick(double min, double max)
{
	...
	if (y < 5)
	{
		dg = 0.5 * pow((double)10, (double)z);
	}
	if (y < 2)
	{
		dg = 0.2 * pow((double)10, (double)z);
	}
...
void MatPlot::display()
{
	...
	while (!is_done)
	{
		...
		{// Configuration of objects
			if (is_debug1)
			{
				std::cout << "============================= configulation ..." << std::endl;
			}
			iFigure = 0;
			iLayer = 0;
			iAxes = 0;
			...
			if (vFigure.size() != iFigure)
			{
				...
				if (is_debug1)
				{
					std::cout << "not match in Figure" << std::endl;
				}
			}
			if (vAxes.size() != iAxes)
			{
				...
				if (is_debug1)
				{
					std::cout << "not match in Axes" << std::endl;
				}
			}
			if (vLine.size() != iLine)
			{
				...
				if (is_debug1)
				{
					std::cout << "not match in Line" << std::endl;
				}
			}
			if (vSurface.size() != iSurface)
			{
				...
				if (is_debug1)
				{
					std::cout << "not match in Surface" << std::endl;
				}
			}
			if (vText.size() != iText)
			{
				...
				if (is_debug1)
				{
					std::cout << "not match in Text" << std::endl;
				}
			}
			if (vLayer.size() != iLayer)
			{
				...
				if (is_debug1)
				{
					std::cout << "not match in Layer" << std::endl;
				}
			}
...
/// set(p,v),set(h,p,v)
void MatPlot::set(std::string p, std::string v)
{
	int h = gco();
	set(h, p, v);
}

void MatPlot::set(std::string p, float v)
{
	int h = gco();
	set(h, p, v);
}

void MatPlot::set(int h, std::string p, std::string v)
{
	int tObj = h % 100;
	int iObj = h / 100;
	// Axes //
...
void MatPlot::set(int h, std::string p, float v)
{
	int tObj = h % 100;
	int iObj = h / 100;
	// Line //
...
int MatPlot::layer(std::string s, int Visible)
{
	int h = layer();
	if (mode == 0)
	{
...
int MatPlot::frame(std::string s, int Visible)
{
	int h = layer();
	if (mode == 0)
	{
...
void MatPlot::Axes_mouse(int button, int state, int x, int y)
{
	...
	{// Left Click
		...
		if (iAxesSelected >= 0)
		{
			...
			if (ca->Visible)
			{
				...
				if ((l <= X) && (X <= l + w) && (b <= Y) && (Y <= b + h) && (ca->Mouse == 1))
				{
					...
					if (is_debug2)
					{
						std::cout << "mouse capture: " << mx << " " << my << std::endl;
					}
				}
			}
		}
		...
		if (is_debug2)
		{
			std::cout << "selected axes" << iAxesSelected << std::endl;
		}
	}// left click
}
...
void MatPlot::axis(std::string s)
{
	if (s == "on")
	{
		ca->Box = 1;
...
void MatPlot::grid(std::string s)
{
	if (s == "on")
	{
		ca->XGrid = 1;
...
void MatPlot::title(std::string s)
{
	ca->Title = s;
}

void MatPlot::xlabel(std::string s)
{
	ca->XLabel = s;
}

void MatPlot::ylabel(std::string s)
{
	ca->YLabel = s;
}
...
int MatPlot::plot(dvec x1, dvec y1, dvec x2, dvec y2)
{
	line(x1, y1);
	return line(x2, y2);
}

int MatPlot::plot(std::valarray<double> x, std::valarray<double> y)
{
	dvec xx, yy;
	for (int i = 0; i < x.size(); ++i)
	{
...
void MatPlot::display_line()
{
	...
	float r;//marker size
	float rx, ry;
	std::vector<float> rgb = ColorSpec2RGB(cl->Color);
	glColor3f(rgb[0], rgb[1], rgb[2]);
		...
		while (mode < 5)
		{
			...
			{//search next contour point
				...
				while (!is && (m < vc.size()))
				{
					...
					{
						if ((c.xy == 0) && (vc[m].xy == 0) && (vc[m].xj == c.xj) && (vc[m].yi == c.yi - 1))
						{
							is = 1;
						}
						if ((c.xy == 0) && (vc[m].xy == 0) && (vc[m].xj == c.xj) && (vc[m].yi == c.yi + 1))
						...
						if ((c.xy == 0) && (vc[m].xy == 1) && (vc[m].xj == c.xj) && (vc[m].yi == c.yi))
						{
							is = 3;
						}
						if ((c.xy == 0) && (vc[m].xy == 1) && (vc[m].xj == c.xj + 1) && (vc[m].yi == c.yi))
						{
							is = 4;
						}
						if ((c.xy == 0) && (vc[m].xy == 1) && (vc[m].xj == c.xj) && (vc[m].yi == c.yi - 1))
						{
							is = 5;
						}
						if ((c.xy == 0) && (vc[m].xy == 1) && (vc[m].xj == c.xj + 1) && (vc[m].yi == c.yi - 1))
						{
							is = 6;
						}
						if ((c.xy == 1) && (vc[m].xy == 1) && (vc[m].xj == c.xj + 1) && (vc[m].yi == c.yi))
						{
							is = 7;
						}
						if ((c.xy == 1) && (vc[m].xy == 1) && (vc[m].xj == c.xj - 1) && (vc[m].yi == c.yi))
						...
						if ((c.xy == 1) && (vc[m].xy == 0) && (vc[m].xj == c.xj) && (vc[m].yi == c.yi))
						{
							is = 9;
						}
						if ((c.xy == 1) && (vc[m].xy == 0) && (vc[m].xj == c.xj) && (vc[m].yi == c.yi + 1))
						{
							is = 10;
						}
						if ((c.xy == 1) && (vc[m].xy == 0) && (vc[m].xj == c.xj - 1) && (vc[m].yi == c.yi))
						{
							is = 11;
						}
						if ((c.xy == 1) && (vc[m].xy == 0) && (vc[m].xj == c.xj - 1) && (vc[m].yi == c.yi + 1))
						{
							is = 12;
						}
					}
...
int MatPlot::text(double x, double y, std::string s)
{
...
/// ptext
void MatPlot::ptext(float x, float y, std::string s)
{
	// Viewport Figure 
	glViewport(0, 0, (int)(window_w), (int)(window_h));
	glLoadIdentity();
...
void MatPlot::ptext3(float x, float y, float z, std::string s)
{
	glColor3f(0, 0, 0);
	glRasterPos3d(x, y, z);
	gl2psText(s.c_str(), "Arial", 12);
...
void MatPlot::ptext3c(float x, float y, float z, std::string s)
{
	int char_w = 6, char_h = 12;
...
void MatPlot::Shading(std::string c)
{
	shading(c);
}
...
std::vector<float> MatPlot::colormap(std::string c, float t)
{
...
void MatPlot::colormap(std::string c)
{
...
void MatPlot::colormap(std::vector<vector<float>> c)
{
	cmap = c;
}
...
std::vector<float> MatPlot::map2color(double x, double xmin, double xmax)
{
	int n = cmap.size();
	float normx;
	std::vector<float> rgb(3);
...
std::vector<float> MatPlot::ColorSpec2RGB(std::string c)
{
	float r, g, b;
	//line
	if (c == "k")
...
std::string MatPlot::rgb2colorspec(std::vector<float> rgb)
{
	char c[100];
